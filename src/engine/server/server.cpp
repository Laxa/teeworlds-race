...
void CServer::ConStatus(IConsole::IResult *pResult, void *pUser)
{
	...
	for(i = 0; i < MAX_CLIENTS; i++)
	{
		...
		{
			const char *pAuthStr = pServer->m_aClients[i].m_Authed == CServer::AUTHED_ADMIN ? "(Admin)" :
										pServer->m_aClients[i].m_Authed == CServer::AUTHED_MOD ? "(Mod)" : "";
			...
			if(pServer->m_aClients[i].m_State == CClient::STATE_INGAME)
				str_format(aBuf, sizeof(aBuf), "id=%d addr=%s name='%s' score=%d client=%d secure=%s %s", i, aAddrStr,
					pServer->m_aClients[i].m_aName, pServer->m_aClients[i].m_Score, 0, pServer->m_NetServer.HasSecurityToken(i) ? "yes":"no", pAuthStr);
			else
